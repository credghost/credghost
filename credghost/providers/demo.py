        ...
        if progress_callback:
            progress_callback(
                ...
                sum(
                    1
                    for i in identities
                    if i.nhi_type in (NHIType.IAM_USER, NHIType.SERVICE_ACCOUNT)
                ),
            )
            progress_callback(
                "roles", sum(1 for i in identities if i.nhi_type == NHIType.IAM_ROLE)
            )
